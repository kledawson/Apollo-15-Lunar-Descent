...
void Octree::subdivide(const ofMesh& mesh, TreeNode& node, int numLevels, int level) {
	...
	// Subdivide the node's box into 8 smaller boxes.
	vector<Box> childBoxes;
	subDivideBox8(node.box, childBoxes);

	// For each child box, find points inside it.
	for (Box& childBox : childBoxes) {
		vector<int> childPoints;
		int pointsInBox = getMeshPointsInBox(mesh, node.points, childBox, childPoints);
		...
		if (pointsInBox > 0) {
			...
			// Recursively subdivide this child node.
...
bool Octree::intersect(const Ray& ray, const TreeNode& node, TreeNode& nodeRtn) {
	if (node.box.intersect(ray, 0.0f, FLT_MAX)) {
...
void Octree::draw(TreeNode& node, int numLevels, int level, const vector<ofColor>& colors) {
	...
	for (TreeNode& child : node.children) {
		draw(child, numLevels, level + 1, colors);
	}
}
